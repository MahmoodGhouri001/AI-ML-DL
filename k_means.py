...
X1 = pd.DataFrame(np.concatenate((X, cluster_val), axis=1), columns = ['Annual Income (k$)', 'Spending Score', 'cluster'])
...
for k in range(final_K):
    plt.scatter(X1.loc[X1['cluster'] == k]['Annual Income (k$)'],X1.loc[X1['cluster'] == k]['Spending Score'],s=100,c=color[k],label=labels[k])
